...
const AllData = ({ onNavigateBack }) => {
    ...
    // Meminta & Menerima data dari server
    useEffect(() => {
        if (isConnected) {
            setIsLoading(true);
            requestAllData({ period: dateFilter });
        }
    }, [isConnected, requestAllData, dateFilter]);
    ...
    // Menerapkan filter client-side
    useEffect(() => {
        let filtered = [...sourceData];
        if (searchTerm) {
            const lowerCaseSearchTerm = searchTerm.toLowerCase();
            filtered = filtered.filter(item =>
                item.output.toString().includes(lowerCaseSearchTerm) ||
        ...
        setCurrentPage(1);
    }, [sourceData, searchTerm, statusFilter]);
    ...
    const handleExportCSV = () => {
        const headers = ['No', 'Start Time', 'End Time', 'Cycle Time (s)', 'Output', 'Status', 'Created At'];
        const csvContent = [
            headers.join(','),
        ...
        <div className="min-h-screen bg-gray-50 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex items-center justify-between">
                    <div><p className="text-sm text-gray-600 mb-1">Total Records</p><p className="text-2xl font-bold text-blue-600">{stats.total}</p></div><BarChart3 className="w-8 h-8 text-blue-400" />
                </div>
                {/* OK Count */}
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex items-center justify-between">
                    <div><p className="text-sm text-gray-600 mb-1">OK Count</p><p className="text-2xl font-bold text-green-600">{stats.totalOK}</p></div><CheckCircle className="w-8 h-8 text-green-400" />
                </div>
                {/* NG Count */}
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex items-center justify-between">
                    <div><p className="text-sm text-gray-600 mb-1">NG Count</p><p className="text-2xl font-bold text-red-600">{stats.totalNG}</p></div><XCircle className="w-8 h-8 text-red-400" />
                </div>
                {/* Success Rate */}
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex items-center justify-between">
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
                    <div className="flex flex-col md:flex-row md:items-center space-y-4 md:space-y-0 md:space-x-4">
                        <div className="flex items-center space-x-2">
                            <Clock className="w-4 h-4 text-gray-400" /><select value={dateFilter} onChange={(e) => setDateFilter(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"><option value="today">Today</option><option value="last7days">Last 7 Days</option><option value="thismonth">This Month</option><option value="all">All Time</option></select>
                        </div>
                        <div className="relative">
                            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" /><input type="text" placeholder="Search data..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 pr-4 py-2 w-full md:w-auto border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                        </div>
                        <div className="flex items-center space-x-2">
                            <Filter className="w-4 h-4 text-gray-400" /><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"><option value="all">All Status</option><option value="ok">OK Only</option><option value="ng">NG Only</option></select>
                        </div>
                    </div>
                    <button onClick={handleExportCSV} className="flex items-center space-x-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors"><Download className="w-4 h-4" /><span>Export CSV</span></button>
                </div>
                <div className="mt-4 text-sm text-gray-600">Showing {filteredData.length > 0 ? startIndex + 1 : 0}-{Math.min(endIndex, filteredData.length)} of {filteredData.length} records.</div>
            </div>
                    ...
                    <table className="w-full">
                        <thead className="border-b border-gray-200 bg-gray-50">
                            <tr>
                                ...
                                <th className="text-left py-4 px-6 text-sm font-semibold text-gray-900">Output</th>
                        ...
                        <tbody>
                            {isLoading ? (
                                <tr><td colSpan="7" className="py-12 text-center text-gray-500"><div className="flex flex-col items-center space-y-2"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div><span>Loading data...</span></div></td></tr>
                            ) : currentData.length === 0 ? (
                                <tr><td colSpan="7" className="py-12 text-center text-gray-500"><div className="flex flex-col items-center space-y-2"><BarChart3 className="w-12 h-12 text-gray-300" /><span>No data found.</span></div></td></tr>
                            ) : (
                                currentData.map((item, index) => (
                                    <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                        <td className="py-4 px-6 text-sm font-medium text-gray-500">{startIndex + index + 1}</td>
                                        ...
                                        <td className="py-4 px-6 text-sm font-medium text-gray-900">{item.output}</td>
